...
type ReadRequest struct {
	...
	// doc_id is the caller's stable document identity (WeKnora knowledge ID).
	// StarKB's parse engine keys the contract package directory by it, and the
	// graph backfill worker resolves the package as <contract_root>/<doc_id>.
	// Engines that do not need a caller-supplied identity ignore it.
	DocId         string `protobuf:"bytes,8,opt,name=doc_id,json=docId,proto3" json:"doc_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *ReadRequest) GetDocId() string {
	if x != nil {
		return x.DocId
	}
	return ""
...
const file_docreader_proto_rawDesc = "" +
	...
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01J\x04\b\x03\x10\x04\"\xf7\x01\n" +
	"\vReadRequest\x12!\n" +
	...
	"\x06doc_id\x18\b \x01(\tR\x05docId\"\xa6\x01\n" +
	"\bImageRef\x12\x1a\n" +
