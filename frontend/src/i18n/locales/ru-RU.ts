export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'StarKB Skill',
      subtitle: 'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API StarKB — загрузки, URL, Markdown и поиск.',
      ...
      installCtaHint: 'Установка StarKB Skill · откроется в новой вкладке',
    ...
    chrome: {
      ...
      subtitle: 'Для self-hosted StarKB: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В StarKB Desktop задайте фиксированный порт API (например 37841) в API-информации.'
        },
        api: {
    ...
    api: {
      ...
      hmacSecretDesc: 'Тот же secret, что в StarKB; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortLabel: 'Режим и сила размышления',
      reasoningEffortPlaceholder: 'По умолчанию модели',
      reasoningEffortDesc: 'Значение по умолчанию для модели: отключить размышление или настроить его силу (автоматически сужается до уровней, поддерживаемых поставщиком). Явная настройка агента/сессии имеет приоритет.',
      thinkingBudgetLabel: 'Бюджет размышления (токены)',
      thinkingBudgetPlaceholder: 'По умолчанию поставщика',
      thinkingBudgetDesc: 'Ограничивает токены размышления на запрос; учитывается только поставщиками с полем бюджета (Anthropic / DashScope / Gemini и др.).',
      reasoning: {
        off: 'Выкл.',
        minimal: 'Минимум',
        low: 'Низкая',
      ...
      embedTuningLabel: 'Настройка эмбеддинга',
      embedBatchSize: 'Размер пакета эмбеддинга',
      embedRetryAttempts: 'Повторы при лимите',
      embedRetryBaseDelayMs: 'Базовая задержка повтора (мс)',
      embedRateLimitDelayMs: 'Нижняя граница задержки при лимите (мс)',
      embedTuningDefault: 'По умолчанию',
      embedTuningDesc: 'Смягчает 429 от поставщиков с жёсткими лимитами (напр. Ark plan account RPM) при фоновой векторизации. Пусто = значения по умолчанию (пакет 5, 8 повторов, задержка от 2 с, при лимите ≥10 с).',
  ...
  chat: {
    ...
    provenance: {
      ...
      badge: 'Источник',
      empty: 'Данные прослеживаемости не были переданы с этим сообщением. Подробности появятся здесь, как только слой выравнивания контрактов запишет якоря блоков.',
      source: 'Источник информации',
      channel: 'Канал',
      reliability: 'Надёжность',
      reliabilityAria: 'Надёжность источника {level} / 5',
      unregistered: 'Не зарегистрирован',
      missing: 'Не предоставлено',
      document: 'Исходный документ',
      documentTitle: 'Документ',
      documentId: 'ID документа',
      knowledgeBase: 'ID базы знаний',
      openDocument: 'Открыть документ',
      location: 'Расположение',
      pageItem: 'Стр. {page}',
      noPages: 'Страницы не переданы',
      chunk: 'Якоря чанков',
      anchorCount: 'Совпавшие якоря',
      anchorCountValue: '{count} блоков',
      noAnchors: 'Якоря не переданы',
      method: 'Метод выравнивания',
      method_exact: 'Точное выравнивание',
      method_fuzzy: 'Нечёткое выравнивание',
      method_failed: 'Ошибка выравнивания',
      chunkId: 'ID чанка',
      excerpt: 'Фрагмент содержимого',
      l5Pending: 'Прослеживаемость строк/столбцов таблиц (L5) требует привязки указателей утверждений при генерации и появится в следующей версии',
      granularitySentence: 'Уровень предложения',
      granularityBlock: 'Уровень блока',
    },
    sandbox: {
  ...
  knowledgeEditor: {
    ...
    wikiBrowser: {
      ...
      tabGraph: 'Граф',
      tabGraphTip: 'Граф связей между Wiki-страницами (граф ссылок страниц). Это НЕ то же самое, что граф знаний на основе сущностей и отношений в обозревателе графа (Настройки БЗ → Открыть обозреватель графа).',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: 'Optional. StarKB POSTs message_sent / message_received events to this HTTPS endpoint.',
  ...
  createChat: {
    title: 'Привет, я StarKB — ваши знания всегда под рукой',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с StarKB',
  ...
  knowledgeGraph: {
    title: 'Обозреватель графа знаний',
    back: 'Назад к базе знаний',
    refresh: 'Обновить',
    loading: 'Загрузка…',
    empty: 'Для этой базы знаний пока нет данных графа',
    unavailable: 'Данные графа недоступны',
    selectHint: 'Нажмите на узел, чтобы увидеть сведения о сущности',
    truncatedNodes: 'Узлов много — показаны {shown} с наибольшим числом связей (из {total})',
    entityType: 'Тип',
    degree: 'Связи',
    noDescription: '(без описания)',
    neighbors: 'Связанные сущности',
    neighborsEmpty: 'Связанных сущностей нет',
    ...
    uncoveredDocs: 'Граф покрывает {ready}/{eligible} документов',
  },
  graphSettings: {
    lightragTitle: 'Граф знаний LightRAG',
    lightragDescription: 'Граф LightRAG на уровне базы знаний: документы автоматически превращаются в граф знаний после разбора и служат каналом графового поиска в вопросах и ответах (с прослеживаемостью доказательств).',
    autoBuildLabel: 'Автоматическое построение графа',
    autoBuildDescription: 'Если включено, разобранные документы этой базы автоматически передаются в построение графа; сбои отображаются ниже и могут быть повторены.',
    buildModelLabel: 'Модель построения графа',
    buildModelDescription: 'Модель, используемая для извлечения графа. Очистите, чтобы использовать модель базы знаний; пакетное извлечение чувствительно к производительности, поэтому можно выбрать быструю модель без размышления.',
    buildModelFollowKb: 'Следовать модели базы знаний',
    graphViewLabel: 'Визуализация графа',
    graphViewEmpty: 'Для этой базы знаний пока нет данных графа',
    openExplorer: 'Открыть обозреватель графа',
    coverageTitle: 'Покрытие графа',
    coverageDescription: 'Прогресс построения графа по документам, пригодным для построения (вставленные документы не имеют контрактной семантики и исключены)',
    coverageReady: 'Построено {ready} из {eligible} документов',
    coveragePending: '{n} в очереди или строятся',
    coverageFailed: '{n} с ошибкой — причины и повтор в бейджах списка документов',
    coverageExempt: '{n} вставленных документов исключены из построения',
    coverageEmpty: 'Данных о покрытии пока нет',
    orphansLine: 'Найдено висячих свидетельств: {n} (ссылки на удалённые документы, затронуто {docs}); устраняется каскадной очисткой или перестроением графа',
    statusTitle: 'Состояние графа',
    statusDescription: 'Масштаб графа и последние задачи построения (из эндпоинта здоровья графового сервиса)',
    statDocs: 'Документов в графе',
    statEntities: 'Сущности',
    statRelations: 'Связи',
    statChunks: 'Чанки графа',
    statusUnavailable: 'Состояние графа недоступно',
    refresh: 'Обновить',
  ...
  envVarSettings: {
    ...
    description: 'Личные ключи для навыков и песочниц, а не системные или деплой-настройки StarKB.',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: 'Держит долгоживущий контейнер на каждую сессию на этом хосте StarKB; скрипты и файлы остаются в том же контейнере',
      },
      ...
      dockerHostRisk: 'Пустое значение или unix:// использует демон Docker на машине StarKB — это равносильно root на этой машине. Только для частной одноузловой установки. Если несколько пространств делят один хост, используйте Cube или E2B. Удалённый tcp:// требует каталог TLS-сертификатов.',
      ...
      weknoraTemplateTitle: 'StarKB standard template',
      weknoraDockerImage: 'StarKB standard image',
      ...
      weknoraTemplateOverview: 'StarKB provides the standard runtime. Templates are discovered after connecting and the standard one is created when missing.',
      weknoraTemplateDescription: 'Includes the Python, Node.js, CLI tools, workspace path, and non-root execution user expected by StarKB skills.',
      ...
      standardTemplateProvisioning: 'The StarKB standard template is being created. Refresh shortly to see its status.',
      ...
      templateUntaggedHint: 'Сборки завершены, но ни одна не имеет тега default, поэтому при создании песочницы шаблон не находится. Удалите его в E2B и обновите список — StarKB пересоберёт шаблон.',
      templateFailedReason: 'Сборка не удалась: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'Стандартный шаблон StarKB',
      ...
      weknoraDesktopTemplate: 'Шаблон рабочего стола StarKB',
      ...
      replaceStandardTemplateConfirm: 'Стандартный шаблон StarKB будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон не удаляется, пока замена не станет готовой к запуску.',
      desktopTemplateProvisioning: 'Шаблон рабочего стола StarKB создаётся. Обновите список чуть позже, чтобы увидеть статус.',
      ...
      replaceDesktopTemplateConfirm: 'Шаблон рабочего стола StarKB будет собран заново с текущими настройками, включая DNS. Предыдущий готовый шаблон рабочего стола не удаляется, пока замена не станет готовой к запуску. Шаблон CLI не затрагивается.',
      ...
      cubeDnsServersHelp: 'Необязательно. IP nameserver, которые записываются в стандартный шаблон StarKB. Пустое значение оставляет значение кластера по умолчанию (обычно 119.29.29.29). Если UDP/53 до публичных резолверов недоступен, укажите достижимые адреса из /etc/resolv.conf хоста Cube, исключая 10/8, 172.16/12 и 192.168/16. Чтобы изменение вступило в силу, на карточке шаблона нажмите «Пересобрать».',
      ...
      dockerTlsCertPathHelp: 'Directory on the StarKB host holding ca.pem, cert.pem and key.pem. Required for a remote daemon; certificates are mounted by the deployment, never stored here.',
      dockerIdleTtl: 'Idle reclaim (seconds)',
      dockerIdleTtlHelp: 'The Docker daemon has no idle timeout of its own. A container that runs no command for this long is reclaimed by StarKB and rebuilt when the session continues. Empty means 1800 seconds.',
  ...
  knowledgeBase: {
    ...
    graphBadge: {
      ...
      tip: {
        none: 'Граф знаний ещё не построен — документ не участвует в поиске по графу',
        pending: 'В очереди, ожидает фонового построения',
        building: 'Построение графа знаний',
        ready: 'Граф знаний готов и участвует в поиске по графу',
        failed: 'Не удалось построить граф — нажмите, чтобы увидеть причину и повторить',
        stale: 'Документ изменён, граф нужно перестроить',
        deleting: 'Удаление данных графа этого документа',
      },
      errorTitle: 'Причина ошибки построения',
      unknownError: 'Причина не записана',
      attempts: 'Повторных попыток: {n}',
      retry: 'Повторить',
      retrying: 'Отправка',
    },
    columnName: 'Имя',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и StarKB автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.'
      },
      welcome: {
        title: 'Добро пожаловать в StarKB',
