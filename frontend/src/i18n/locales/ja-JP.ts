export default {
  ...
  knowledgeBase: {
    ...
    graphBadge: {
      ...
      tip: {
        none: 'ナレッジグラフ未構築のため、グラフ検索の対象外です',
        pending: '待機中です。バックグラウンドで構築されます',
        building: 'ナレッジグラフを構築しています',
        ready: 'ナレッジグラフ構築済み。グラフ検索の対象です',
        failed: '構築に失敗しました。クリックで原因を確認して再試行できます',
        stale: '文書が更新されたため、グラフの再構築が必要です',
        deleting: 'この文書のグラフデータを削除しています',
      },
      errorTitle: '構築失敗の原因',
      unknownError: '原因は記録されていません',
      attempts: '{n} 回再試行済み',
      retry: '再試行',
      retrying: '送信中',
    },
  ...
  knowledgeGraph: {
    ...
    refresh: '更新',
    loading: '読み込み中…',
    empty: 'このナレッジベースにはまだグラフデータがありません',
    unavailable: 'グラフデータを利用できません',
    selectHint: 'ノードをクリックするとエンティティの詳細を表示します',
    truncatedNodes: 'ノードが多いため、次数の高い上位 {shown} 件を表示しています（全 {total} 件）',
    entityType: '種類',
    degree: '接続数',
    noDescription: '（説明なし）',
    neighbors: '関連エンティティ',
    neighborsEmpty: '関連エンティティはありません',
    evidence: '根拠',
    ...
    uncoveredDocs: 'グラフは {eligible} 件中 {ready} 件のドキュメントをカバー',
  },
  graphSettings: {
    ...
    buildModelLabel: 'グラフ構築モデル',
    buildModelDescription: 'グラフ抽出に使用するモデル。クリアするとナレッジベースのモデルに従います。バッチ抽出はスループットが重要なため、高速/非思考モデルを個別に指定できます。',
    buildModelFollowKb: 'ナレッジベースのモデルに従う',
    graphViewLabel: 'グラフ可視化',
    graphViewEmpty: 'このナレッジベースにはまだグラフデータがありません',
    openExplorer: 'グラフブラウザーを開く',
    coverageTitle: 'グラフカバレッジ',
    coverageDescription: '構築可能なドキュメントのグラフ構築進捗（貼り付けドキュメントは契約セマンティクスを持たないため対象外）',
    coverageReady: '{eligible} 件中 {ready} 件のグラフ構築済み',
    coveragePending: '{n} 件が待機・構築中',
  ...
  knowledgeEditor: {
    ...
    wikiBrowser: {
      ...
      tabDocuments: 'ドキュメント',
      tabGraph: 'グラフ',
      tabGraphTip: 'Wikiページ間のリンクを表したグラフ（ページリンクグラフ）です。ナレッジグラフブラウザー（ナレッジベース設定 → グラフブラウザーを開く）で扱う、LLMが抽出するエンティティ・リレーションのナレッジグラフとは異なります。',
  ...
  chat: {
    ...
    provenance: {
      title: '出所トレース',
      badge: '出所',
      empty: '出所データがこのメッセージに含まれていません。契約アライメント層がブロックアンカーを記録すると、ここに詳細が表示されます。',
      source: '情報源',
      channel: 'チャネル',
      reliability: '信頼度',
      reliabilityAria: '情報源の信頼度 {level} / 5',
      unregistered: '未登録',
      missing: '未提供',
      document: '出典文書',
      documentTitle: '文書',
      documentId: '文書 ID',
      knowledgeBase: 'ナレッジベース ID',
      openDocument: '文書を開く',
      location: '章節位置',
      pageItem: '{page} ページ',
      noPages: 'ページ情報は未提供',
  ...
  system: {
    ...
    versionDescription: 'アプリケーションサービスのバージョン',
    frontendVersionLabel: 'UIバージョン',
    frontendVersionDescription: 'UIのビルドバージョン',
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortLabel: '思考モードと強度',
      reasoningEffortPlaceholder: 'モデル既定に従う',
      reasoningEffortDesc: 'モデル行の既定値：思考を無効化するか強度を調整します（ベンダーが対応するレベルへ自動的に縮小）。エージェント/セッションの明示設定が優先されます。',
      thinkingBudgetLabel: '思考バジェット（tokens）',
      thinkingBudgetPlaceholder: 'ベンダー既定',
      thinkingBudgetDesc: 'リクエストごとの思考トークン上限。バジェット項目を持つベンダー（Anthropic / DashScope / Gemini など）のみ有効です。',
      reasoning: {
        off: 'オフ',
        minimal: '最低',
        low: '低',
      ...
      embedTuningLabel: '埋め込みチューニング',
      embedBatchSize: '埋め込みバッチサイズ',
      embedRetryAttempts: 'レート制限リトライ回数',
      embedRetryBaseDelayMs: 'リトライ基本遅延 (ms)',
      embedRateLimitDelayMs: 'レート制限遅延下限 (ms)',
      embedTuningDefault: 'デフォルト',
      embedTuningDesc: 'レート制限が厳しいプロバイダー（Ark plan など）でのバックグラウンド埋め込み 429 を抑制します。空欄でデフォルト（バッチ 5、リトライ 8 回、2s 起点の退避、レート制限時 ≥10s）。',
