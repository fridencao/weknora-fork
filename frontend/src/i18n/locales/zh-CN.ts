export default {
  localBrowser: {
    ...
    "copyPairing": "复制配对链接",
    "copied": "已复制",
    "windowHint": "任务在 Chrome 的 StarKB 标签组内运行。借用已有标签页需确认。",
    "preview": "本机浏览器任务预览",
    "waiting": "等待任务页面",
  ...
  integrations: {
    ...
    claw: {
      title: 'StarKB Skill',
      subtitle: '通过 StarKB REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 StarKB Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 StarKB 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 StarKB 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 StarKB 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 StarKB 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  agentEditor: {
    retrieval: {
      ...
      on: '开',
      off: '关',
      graphChannelInheritBadge: '继承中（当前值：{value}）',
      graphChannelCustomBadge: '已自定义（部署默认：{value}）',
      graphChannelRestore: '恢复继承',
    },
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortLabel: '思考模式与强度',
      reasoningEffortPlaceholder: '跟随模型自身默认',
      reasoningEffortDesc: '模型级默认：关闭思考或调整思考强度（请求会按厂商支持的档位自动收窄）。对话中 Agent/会话的显式设置优先于此。',
      thinkingBudgetLabel: '思考预算（tokens）',
      thinkingBudgetPlaceholder: '厂商默认',
      thinkingBudgetDesc: '限制单次请求的思考 token 用量，仅对暴露预算字段的厂商生效（Anthropic / DashScope / Gemini 等）。',
      reasoning: {
        off: '关闭思考',
        minimal: '最低',
        low: '低',
      ...
      embedTuningLabel: '向量嵌入调优',
      embedBatchSize: '嵌入批次大小',
      embedRetryAttempts: '限流重试次数',
      embedRetryBaseDelayMs: '重试退避基数 (ms)',
      embedRateLimitDelayMs: '限流退避下限 (ms)',
      embedTuningDefault: '默认',
      embedTuningDesc: '治理后台向量化对限流敏感供应商（如火山方舟 plan 账号级 RPM）的 429。留空=默认（批次 5、重试 8 次、退避 2s 起、限流退避 ≥10s）。',
  ...
  chat: {
    ...
    provenance: {
      title: '溯源',
      badge: '溯源',
      empty: '溯源数据未随消息下发。等后端契约归一层写入区块锚点后，这里会展示五级溯源详情。',
      source: '信息源',
      channel: '渠道',
      reliability: '可靠度',
      reliabilityAria: '源可靠度 {level} / 5',
      unregistered: '未注册',
      missing: '未提供',
      document: '源文档',
      documentTitle: '文档',
  ...
  knowledgeEditor: {
    ...
    wiki: {
      ...
      batchIngestLabel: '批量重建 Wiki 页面',
      batchIngestTip: 'KB 启用 Wiki 后，已解析完成的存量文档不会自动生成 Wiki 页面。本按钮把所有 enabled 文档批量入队 wiki:ingest 异步生成页面，预计 KB 内 14 篇大文档耗时 30-90 分钟；可在 Wiki 浏览器中实时看到。',
      batchIngestCta: '重新生成 Wiki 页面',
      batchIngestPolling: '正在生成，剩余 {n} 篇待处理…',
      batchIngestConfirmTitle: '确认批量重建',
      batchIngestConfirmBody: '对知识库「{name}」内所有已解析完成的文档重新生成 Wiki 页面。已存在的 Wiki 页面会被重建为最新摘要，链接与会保留。',
      batchIngestConfirmHint: '新增/未完成文档将自动跳过（不入队）。',
      batchIngestConfirmCost: '估算耗时与文档量成正比；中等规模（约 50 篇）通常 30-90 分钟。',
      batchIngestConfirmCta: '开始重建',
      batchIngestNoKbId: '请先保存知识库后再重建 Wiki 页面。',
      batchIngestSuccess: '已入队 {queued}/{total} 篇，正在处理中。',
      batchIngestFailed: '批量重建失败，请稍后重试。',
      batchIngestLastResult: '上次：入队 {queued} / 跳过 {skipped} / 总计 {total}。',
  ...
  embedPublish: {
    ...
    originsHint: '填写允许嵌入的宿主网站来源（A），不是 StarKB 地址（B）。每行一个完整 Origin，如 https://shop.example.com；至少一项，支持 *.example.com 子域通配',
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，StarKB 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  auth: {
    ...
    firstTime: '首次使用 StarKB？',
    registerSuccess: '注册成功，请登录',
    registerFailed: '注册失败',
    subtitle: 'RAG 问答、ReAct 智能体与 Wiki 知识库，大模型驱动的企业级知识框架',
    registerSubtitle: '创建账户并开始使用 StarKB',
  ...
  knowledgeGraph: {
    ...
    empty: '该知识库暂无图谱数据',
    unavailable: '图谱数据不可用',
    selectHint: '点击节点查看实体详情',
    truncatedNodes: '节点较多，仅显示度数最高的 {shown} 个（共 {total} 个）',
    entityType: '类型',
    degree: '连接数',
    noDescription: '（无描述）',
    neighbors: '关联实体',
    neighborsEmpty: '无关联实体',
    evidence: '证据出处',
    ...
    docFilterClear: '查看全部',
    uncoveredDocs: '图谱覆盖 {ready}/{eligible} 篇文档',
    searchPlaceholder: '搜索实体，定位到图谱',
    searchNoResults: '未找到实体',
    focusCenter: '以此为中心聚焦',
    egoBanner: '中心视图：{center}（{depth} 跳内 {n} 个实体）',
    backToOverview: '返回全图',
    typeFilterClear: '清除类型过滤',
    sourceContext: '展开抽取原文',
    docSwitcherAll: '全部实体（按文档过滤）',
    askEntity: '问这个实体',
        chartAssets: '关联图表资产',
    mergeInto: '合并到其他实体',
    mergeTitle: '合并「{name}」',
    mergeConfirm: '合并',
    mergeWarning: '把该实体并入目标实体：关系与描述将转移，本实体被删除，操作不可逆。',
    mergeTargetPlaceholder: '搜索目标实体，或直接输入新名称（改名）',
    healthTitle: '图谱数据健康',
    healthIssues: '{n} 篇建图失败',
    healthFailed: '建图失败（{n}）',
    healthRetry: '重试',
    healthNone: '无异常',
    healthOrphans: '孤儿证据巡检',
    healthOrphanSummary: '巡检 {keys} 条证据键，悬空 {docs} 篇文档。',
  },
  graphSettings: {
    ...
    buildModelLabel: '建图模型',
    buildModelDescription: '图谱抽取使用的模型。清空 = 跟随知识库模型；批量抽取是性能敏感场景，可单独选用关思考/快速模型。',
    workspaceModeLabel: '图谱空间',
    workspaceModeDescription: '本知识库图谱空间策略：默认 = 跟随系统环境；隔离 = 独立图谱空间（与其它 KB 数据物理隔离）；共享 = 强制并入全局空间（多 KB 图谱合并，存量 KB 用）。切档后需触发图谱重抽取以匹配新空间。',
    workspaceModeDefault: '跟随系统默认',
    workspaceModeShared: '共享全局图谱空间',
    workspaceModeKb: '按本知识库隔离',
    engineHintTitle: '解析引擎提示',
    engineHintBody: '仅 StarKB 引擎产物会写入图谱契约包，其它引擎（MinerU / anydoc / DocReader builtin 等）解析完成的文档不会进图谱。如需图谱通道稳定覆盖，把解析引擎切到 StarKB 全管道即可。',
    engineHintInconsistent: '当前知识库的解析引擎规则没有全部锁定 StarKB，请到知识库设置的「解析引擎」区域统一为 StarKB。',
    engineHintUnsupCount: '估算有 {n} 篇 eligible 文档不会被建图（inferred engine ≠ starkb）。重解析后即可纳入。',
    buildModelFollowKb: '跟随知识库模型',
    graphViewLabel: '图谱可视化',
    graphViewEmpty: '该知识库暂无图谱数据',
    openExplorer: '打开图谱浏览器',
    coverageTitle: '图谱覆盖',
    coverageDescription: 'KB 内可建图文档的图谱构建进度（粘贴类文档无契约语义，不计入）',
    coverageReady: '已建图 {ready} / {eligible} 篇',
    coveragePending: '{n} 篇排队或建图中',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是 StarKB 的系统或部署配置。',
    helpAria: '沙箱密钥说明',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用 StarKB 所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: 'StarKB 标准模板',
      weknoraDockerImage: 'StarKB 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: 'StarKB 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 StarKB 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      standardTemplateProvisioning: '已开始在集群中创建 StarKB 标准模板，请稍后刷新查看状态',
      ...
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 StarKB 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: 'StarKB 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      weknoraDesktopTemplate: 'StarKB 桌面模板',
      createDesktopTemplate: '创建',
      createDesktopTemplateHint: '从官方桌面镜像构建 XFCE 图形桌面。体积明显大于标准模板，仅在需要图形界面时创建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 StarKB 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      desktopTemplateProvisioning: '已开始在集群中创建 StarKB 桌面模板，请稍后刷新查看状态',
      desktopTemplateReplaced: '已删除原桌面模板并开始重建，请等待状态就绪。标准（CLI）模板不受影响。',
      desktopTemplateTag: '桌面',
      replaceDesktopTemplateConfirm: '将用当前配置（含 DNS）重建 StarKB 桌面模板。新模板就绪前不会删除仍可启动的旧桌面模板，也不会删除标准（CLI）模板。',
      ...
      cubeDnsServersHelp: '可选。写入 StarKB 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'StarKB 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 StarKB 回收，会话继续时重建。留空按 1800 秒。',
    ...
    parser: {
      ...
      languagePlaceholder: '选择识别语言，默认 ch（简体中文）',
  ...
  knowledgeBase: {
    ...
    graphBadge: {
      ...
      tip: {
        none: '尚未生成知识图谱，该文档暂不参与图谱检索',
        pending: '已排队，等待后台建图',
        building: '正在生成知识图谱',
        ready: '已生成知识图谱，可参与图谱检索',
        failed: '建图失败，点击查看原因并重试',
        stale: '文档已更新，图谱待重建',
        deleting: '正在清理该文档的图谱数据',
      },
      errorTitle: '建图失败原因',
      unknownError: '未记录具体原因',
      attempts: '已重试 {n} 次',
      retry: '重试',
      retrying: '提交中',
    },
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，StarKB 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
        title: '欢迎使用 StarKB',
