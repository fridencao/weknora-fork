export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'StarKB Skill',
      subtitle: 'StarKB REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.',
      ...
      installCtaHint: 'StarKB Skill 설치 · 새 탭에서 열림',
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: '자체 호스팅 StarKB와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.',
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: 'StarKB 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC secret은 StarKB와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=weknora, exp(최대 24시간)가 필요합니다.',
      ...
      hmacSecretDesc: 'StarKB에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.',
  ...
  model: {
    ...
    editor: {
      ...
      reasoningEffortLabel: '사고 모드 및 강도',
      reasoningEffortPlaceholder: '모델 기본값 따름',
      reasoningEffortDesc: '모델 행 기본값: 사고를 비활성화하거나 강도를 조정합니다(벤더가 지원하는 수준으로 자동 축소). 에이전트/세션의 명시적 설정이 우선합니다.',
      thinkingBudgetLabel: '사고 예산 (tokens)',
      thinkingBudgetPlaceholder: '벤더 기본값',
      thinkingBudgetDesc: '요청당 사고 토큰 상한. 예산 필드를 제공하는 벤더(Anthropic / DashScope / Gemini 등)에서만 적용됩니다.',
      reasoning: {
        off: '끄기',
        minimal: '최소',
        low: '낮음',
      ...
      embedTuningLabel: '임베딩 튜닝',
      embedBatchSize: '임베딩 배치 크기',
      embedRetryAttempts: '레이트 리밋 재시도 횟수',
      embedRetryBaseDelayMs: '재시도 기본 지연 (ms)',
      embedRateLimitDelayMs: '레이트 리밋 지연 하한 (ms)',
      embedTuningDefault: '기본값',
      embedTuningDesc: '레이트 리밋이 엄격한 공급자(예: Ark plan 계정 RPM)의 백그라운드 임베딩 429을 완화합니다. 비워두면 기본값(배치 5, 재시도 8회, 2s 기반 백오프, 레이트 리밋 시 ≥10s).',
  ...
  chat: {
    ...
    provenance: {
      title: '출처 추적',
      badge: '출처',
      empty: '출처 데이터가 이 메시지와 함께 전달되지 않았습니다. 계약 정렬 계층이 블록 앵커를 기록하면 여기에 상세 정보가 표시됩니다.',
      source: '정보원',
      channel: '채널',
      reliability: '신뢰도',
      reliabilityAria: '정보원 신뢰도 {level} / 5',
      unregistered: '미등록',
      missing: '미제공',
      document: '출처 문서',
      documentTitle: '문서',
      documentId: '문서 ID',
      knowledgeBase: '지식베이스 ID',
      openDocument: '문서 열기',
      location: '위치',
      pageItem: '{page}페이지',
      noPages: '페이지 정보 미제공',
  ...
  knowledgeEditor: {
    ...
    wikiBrowser: {
      ...
      tabDocuments: '문서',
      tabGraph: '그래프',
      tabGraphTip: 'Wiki 페이지 간의 링크 관계 그래프(페이지 링크 그래프)입니다. 지식 그래프 탐색기(지식베이스 설정 → 그래프 탐색기 열기)에서 다루는 LLM 기반 엔티티-관계 지식 그래프와는 다른 개념입니다.',
  ...
  embedPublish: {
    ...
    originsHint: 'StarKB 주소(B)가 아닌 임베드를 허용할 호스트 웹사이트(A)를 입력하세요. 한 줄에 하나의 Origin(예: https://shop.example.com), 최소 1개. *.example.com 지원.',
  ...
  createChat: {
    title: '안녕하세요, StarKB입니다 — 당신의 지식을 손끝에',
  ...
  auth: {
    ...
    registerSubtitle: '계정을 만들고 StarKB를 시작하세요',
  ...
  knowledgeGraph: {
    ...
    refresh: '새로 고침',
    loading: '불러오는 중…',
    empty: '이 지식베이스에는 아직 그래프 데이터가 없습니다',
    unavailable: '그래프 데이터를 사용할 수 없습니다',
    selectHint: '노드를 클릭하면 엔티티 상세를 볼 수 있습니다',
    truncatedNodes: '노드가 많아 연결 수가 높은 상위 {shown}개만 표시합니다 (전체 {total}개)',
    entityType: '유형',
    degree: '연결 수',
    noDescription: '(설명 없음)',
    neighbors: '관련 엔티티',
    neighborsEmpty: '관련 엔티티가 없습니다',
    evidence: '근거',
    ...
    uncoveredDocs: '그래프 커버리지 {ready}/{eligible}편',
  },
  graphSettings: {
    ...
    buildModelLabel: '그래프 구축 모델',
    buildModelDescription: '그래프 추출에 사용되는 모델입니다. 비우면 지식 베이스 모델을 따릅니다. 배치 추출은 처리량에 민감하므로 빠른/비사고 모델을 별도로 지정할 수 있습니다.',
    buildModelFollowKb: '지식 베이스 모델 따름',
    graphViewLabel: '그래프 시각화',
    graphViewEmpty: '이 지식베이스에는 아직 그래프 데이터가 없습니다',
    openExplorer: '그래프 탐색기 열기',
    coverageTitle: '그래프 커버리지',
    coverageDescription: '구축 가능한 문서 대상 그래프 구축 진행률(붙여넣기 문서는 계약 시맨틱이 없어 제외)',
    coverageReady: '{eligible}편 중 {ready}편 구축 완료',
    coveragePending: '{n}편 대기 또는 구축 중',
  ...
  envVarSettings: {
    title: '샌드박스 키',
    description: '스킬과 샌드박스에 쓰는 개인 키이며, StarKB 시스템이나 배포 설정이 아닙니다.',
    helpAria: '샌드박스 키 설명',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '이 StarKB 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      },
      ...
      dockerHostRisk: '비우거나 unix:// 를 쓰면 StarKB가 있는 머신의 Docker 데몬을 사용하며, 그 권한은 해당 머신의 root와 같습니다. 프라이빗 단일 노드에만 쓰세요. 여러 워크스페이스가 같은 호스트를 쓰는 경우에는 Cube 또는 E2B를 쓰세요. 원격 tcp:// 는 TLS 인증서 디렉터리가 필요합니다.',
      addConfig: '샌드박스 추가',
      ...
      templateUntaggedHint: '빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 StarKB가 다시 빌드합니다.',
      templateFailedReason: '빌드 실패: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'StarKB 표준 템플릿',
      createStandardTemplate: '만들기',
      createStandardTemplateHint: '현재 연결 설정(DNS 포함)으로 구축합니다. 설정을 바꾼 뒤에는 카드에서 재구축하세요.',
      weknoraDesktopTemplate: 'StarKB 데스크톱 템플릿',
      ...
      replaceStandardTemplateConfirm: '현재 설정(DNS 포함)으로 StarKB 표준 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 템플릿을 삭제하지 않습니다.',
      desktopTemplateProvisioning: 'StarKB 데스크톱 템플릿을 만들고 있습니다. 잠시 후 새로고침해 상태를 확인하세요.',
      desktopTemplateReplaced: '기존 데스크톱 템플릿을 삭제했고 재구축을 시작했습니다. 준비될 때까지 기다리세요. CLI 템플릿은 그대로 둡니다.',
      desktopTemplateTag: '데스크톱',
      replaceDesktopTemplateConfirm: '현재 설정(DNS 포함)으로 StarKB 데스크톱 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 데스크톱 템플릿을 삭제하지 않으며, CLI 템플릿은 그대로 둡니다.',
      ...
      cubeDnsServersHelp: '선택 사항입니다. StarKB 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.',
  ...
  knowledgeBase: {
    ...
    graphBadge: {
      ...
      tip: {
        none: '지식 그래프가 아직 없어 그래프 검색에 사용되지 않습니다',
        pending: '대기 중이며 백그라운드에서 구축됩니다',
        building: '지식 그래프를 구축하는 중입니다',
        ready: '지식 그래프가 준비되어 그래프 검색에 사용됩니다',
        failed: '구축에 실패했습니다. 클릭하여 원인을 확인하고 재시도하세요',
        stale: '문서가 변경되어 그래프를 다시 구축해야 합니다',
        deleting: '이 문서의 그래프 데이터를 정리하는 중입니다',
      },
      errorTitle: '구축 실패 원인',
      unknownError: '기록된 원인이 없습니다',
      attempts: '{n}회 재시도함',
      retry: '재시도',
      retrying: '제출 중',
    },
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: '모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 StarKB가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.'
      },
      welcome: {
        title: 'StarKB에 오신 것을 환영합니다',
