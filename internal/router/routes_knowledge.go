...
func RegisterKnowledgeBaseRoutes(r *gin.RouterGroup, handler *handler.KnowledgeBaseHandler, g *rbacGuards) {
	...
	kbManagement := kb.With(apiKeyManageKnowledgeBases(apiKeyFullAccess()))
	{
		...
		// M6-1 WS1.5：覆盖进度（分母在 WeKnora 侧，状态计数在 starkb-api，此层合流）
		kb.GET("/:id/graph/coverage", g.Viewer(), g.KBAccessRead("id"),
			handler.GetKnowledgeBaseGraphCoverage)
		...
		kb.POST("/:id/graph/doc-status", g.Viewer(), g.KBAccessRead("id"),
			handler.GetKnowledgeBaseGraphDocStatus)
		kbManagement.POST("/:id/graph/doc-retry", g.OwnedKBOrAdmin(), g.KBAccessWrite("id"),
			handler.RetryKnowledgeBaseGraphDocs)
