...
// resolveAppCredentials 从模型自身参数中取出 AppID/AppSecret（AppSecret 存储
// 态为密文，此处统一解密）。
func (s *modelService) resolveAppCredentials(params *types.ModelParameters) (appID, appSecret string) {
...
func (s *modelService) GetEmbeddingModel(ctx context.Context, modelId string) (embedding.Embedder, error) {
	...
	appID, appSecret := s.resolveAppCredentials(&model.Parameters)
